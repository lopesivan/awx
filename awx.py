...
VERSION = "1.0.0"
DEFAULT_BASE_URL = "https://seu-servidor.com/wxwidgets"
...
Exemplos:
  ...
  awx remove windows 3.2.4        Remove wxWidgets 3.2.4 para Windows
    ...
    parser.add_argument('--version', action='version', version=f'awx {VERSION}')
    parser.add_argument('--base-url', help=f'URL base do servidor (padrão: {DEFAULT_BASE_URL})')
    parser.add_argument('--install-dir', help=f'Diretório de instalação (padrão: {DEFAULT_INSTALL_DIR})')
    
    subparsers = parser.add_subparsers(dest='command', help='Comandos disponíveis')
    
    # list-available
    subparsers.add_parser('list-available', help='Lista compilações disponíveis no servidor')
    
    # list-installed
    subparsers.add_parser('list-installed', help='Lista compilações instaladas')
    
    # install
    install_parser = subparsers.add_parser('install', help='Instala uma compilação')
    install_parser.add_argument('platform', choices=['linux', 'windows', 'android'], 
                                help='Plataforma alvo')
    install_parser.add_argument('version', help='Versão do wxWidgets (ex: 3.2.4)')
    install_parser.add_argument('variant', nargs='?', 
                                help='Variante (cmake para linux, debug/release para android)')
    
    # remove
    remove_parser = subparsers.add_parser('remove', help='Remove uma compilação instalada')
    remove_parser.add_argument('platform', choices=['linux', 'windows', 'android'],
                               help='Plataforma alvo')
    remove_parser.add_argument('version', help='Versão do wxWidgets (ex: 3.2.4)')
